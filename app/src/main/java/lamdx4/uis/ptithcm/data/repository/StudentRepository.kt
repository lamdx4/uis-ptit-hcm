...
class StudentRepository {
    private val client = HttpClient(CIO) {
        ...
        install(Logging) {
            ...
            level = LogLevel.BODY // hoặc BODY để log cả request/response body
        }
    }
    ...
    suspend fun getStudentDetail(accessToken: String): StudentDetailResponse {
        return client.post("http://uis.ptithcm.edu.vn/api/dkmh/w-locsinhvieninfo") {
            header(HttpHeaders.Authorization, "Bearer $accessToken")
            header(HttpHeaders.Accept, "application/json, text/plain, */*")
            header(HttpHeaders.ContentType, ContentType.Text.Plain)
            setBody("")
    ...
    suspend fun getCompleteStudentInfo(accessToken: String, maSV: String): CompleteStudentInfo {
        return coroutineScope {
            ...
            if (!imageInfo.result || !detailInfo.result) {
            ...
            CompleteStudentInfo(
                ...
                image = imageData.image, // Ảnh từ API image
